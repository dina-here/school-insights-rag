...
### ⚠️ Viktigt: CSV-data är låst för slutanvändare

**Slutanvändare kan INTE:**
- Ladda upp nya CSV-filer genom webb-gränssnittet
- Redigera eller ersätta befintliga CSV-filer online
- Ändra vilka data som är tillgängliga i systemet

**Endast utvecklare kan** (genom att ladda ner projektet från GitHub):
1. `git clone` projektet lokalt
2. Placera nya CSV-filer i `data/`-mappen
3. Kör `python ingest_school_data_postgres.py data/` för att ladda upp nya data
4. Pusha ändringar till GitHub/Render för att uppdatera produktionen

**Så här gör utvecklare för att uppdatera data:**
```bash
# 1. Hämta senaste koden
git clone https://github.com/...
cd SkolAnalys

# 2. Lägg nya CSV-filer i data/-mappen
cp ny_fil.csv data/

# 3. Ställ in .env-fil med API-nycklar och DATABASE_URL

# 4. Ladda upp nya data till PostgreSQL
python ingest_school_data_postgres.py data/

# 5. Pusha ändringar
git push origin main
# Render deployment uppdateras automatiskt
```
